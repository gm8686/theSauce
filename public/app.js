  ...
  for (const side of game.sides) {
    ...
    card.innerHTML = `
      ...
      <div class="trade-actions">
        <button type="button" class="trade-button buy-button" aria-label="Buy ${side.name}" title="Buy ${side.name}"></button>
        <button type="button" class="trade-button sell-button" aria-label="Sell ${side.name}" title="Sell ${side.name}"></button>
      </div>
    `;
...
async function submitTrade(side, action, button) {
  ...
  try {
    ...
    if (result.dryRun) {
      ...
      const details = result.trade?.nominalUsd ? dollarLabel(result.trade.nominalUsd) : action === "buy" ? `$${buyNotionalUsd.toFixed(2)}` : "all available shares";
